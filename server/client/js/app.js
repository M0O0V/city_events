...
async function filterEventsByType(type) {
    if (!type) {
        alert('Тип мероприятия не указан.');
        return;
    }

    try {
        const response = await fetch(`/api/events?type=${encodeURIComponent(type)}`);
        if (!response.ok) {
            // Получаем статус и сообщение об ошибке с сервера (если есть)
            const errorMessage = await response.text();
            throw new Error(`Ошибка загрузки мероприятий: ${response.status} - ${errorMessage}`);
        }

        const events = await response.json();
        
        if (!events || events.length === 0) {
            alert('Мероприятия данного типа не найдены.');
            return;
        }

        displayEvents(events); // Отображаем отфильтрованные мероприятия
    } catch (error) {
        console.error('Ошибка фильтрации мероприятий:', error);
        alert(`Не удалось выполнить фильтрацию: ${error.message}`);
    }
}

// Добавление обработчиков клика на элементы карусели
function initializeCarouselFilters() {
    ...
    carouselItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (!eventType) {
                alert('Тип мероприятия отсутствует. Проверьте элементы карусели.');
                return;
            }
...
toggleFiltersButton.addEventListener('click', () => {
    ...
    updateMenuButton();
    initializeCarouselFilters(); // Инициализируем обработчики для карусели
});
